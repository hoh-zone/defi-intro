...
public fun create_pool<A, B>(coin_a: Coin<A>, coin_b: Coin<B>, fee_bps: u64, ctx: &mut TxContext) {
...
public fun add_liquidity<A, B>(
    ...
    let shares = if (pool.total_supply == 0) {
        ...
        let shares_a =
            (
                (amount_a as u128) * (pool.total_supply as u128)
                / (pool.reserve_a as u128),
            ) as u64;
        let shares_b =
            (
                (amount_b as u128) * (pool.total_supply as u128)
...
public fun remove_liquidity<A, B>(
    ...
    let amount_a =
        (
            (shares as u128) * (pool.reserve_a as u128)
            / (pool.total_supply as u128),
        ) as u64;
    let amount_b =
        (
            (shares as u128) * (pool.reserve_b as u128)
            / (pool.total_supply as u128),
        ) as u64;
...
public fun amount_out(amount_in: u64, reserve_in: u64, reserve_out: u64, fee_bps: u64): u64 {
    assert!(amount_in > 0, EInvalidAmount);
...
fun mint_protocol_fee<A, B>(pool: &mut Pool<A, B>) {
    ...
    let numerator =
        (pool.total_supply as u128) * (root_k_current - root_k_last)
...
public fun set_protocol_fee<A, B>(_cap: &AdminCap, pool: &mut Pool<A, B>, protocol_fee_bps: u64) {
    assert!(protocol_fee_bps <= 10000, EInvalidAmount);
    pool.protocol_fee_bps = protocol_fee_bps;
}

/// Pause or unpause the pool.
public fun set_paused<A, B>(_cap: &AdminCap, pool: &mut Pool<A, B>, paused: bool) {
    pool.paused = paused;
}
