...
public fun pause_all(_: &PauseCap, state: &mut PauseState, reason: u8, clock: &Clock) {
...
public fun pause_deposits(_: &PauseCap, state: &mut PauseState, clock: &Clock) {
    state.deposits_paused = true;
    state.paused_at = sui::clock::timestamp_ms(clock);
}

public fun unpause_all(_: &UnpauseCap, state: &mut PauseState, clock: &Clock) {
