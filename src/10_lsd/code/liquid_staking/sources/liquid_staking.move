...
fun calculate_exchange_rate(pool: &StakingPool): u64 {
    ...
    (
        (balance::value(&pool.total_sui) as u128) * (PRECISION as u128) / (pool.total_lst_minted as u128),
    ) as u64
}
...
public fun stake(
    ...
    let lst_amount = if (pool.total_lst_minted == 0) {
        ...
        (
            (sui_amount as u128) * (pool.total_lst_minted as u128) / (balance::value(&pool.total_sui) as u128),
        ) as u64
    };
...
public fun unstake(
    ...
    let sui_amount =
        ((lst_amount as u128) * (total_sui_value as u128) / (pool.total_lst_minted as u128)) as u64;
...
public fun preview_unstake(pool: &StakingPool, lst_amount: u64): u64 {
    ...
    (
        (lst_amount as u128) * (balance::value(&pool.total_sui) as u128) / (pool.total_lst_minted as u128),
    ) as u64
}
