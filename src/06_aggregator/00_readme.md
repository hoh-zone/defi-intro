...
- 理解报价、路由、结算三层架构。
- 掌握链上 Router、SwapContext 与 DEX adapter 的职责边界。
- 理解多跳、拆单、滑点、Gas 与包版本管理的工程问题。
- 能看懂聚合器 PTB 如何把多个协议调用打包成一笔交易。

## 先修知识

- 读过第 4 章 DEX 的多池路由与第 5 章预言机风险。
- 能阅读 TypeScript 中的交易组装和 Move 中的对象传递。

## 本章小结

聚合器不是一个更聪明的 swap 函数，而是链下搜索和链上结算的协作系统。真正的难点在于让链下报价、链上余额、包版本和用户滑点约束保持一致。

## 练习题

1. 说明为什么多跳最后一跳常需要处理估算误差。
2. 画出 Quote API 返回到 PTB moveCall 的数据流。
3. 列出拆单路由相比单路径路由多出的两类风险。
4. 设计一个聚合器失败时的最小错误提示字段。

## 下一章连接

价格基础设施之后，第三篇进入信用与货币，先从借贷协议开始。
