...
entry fun unstake<StakeCoin, RewardCoin>(
    ...
    assert!(amount > 0, EZeroAmount);
    assert!(bag::contains(&pool.stakes, user_addr), ENoStakeFound);
...
public entry fun claim<StakeCoin, RewardCoin>(
    ...
    let pending =
        user_stake.amount * pool.acc_reward_per_share / PRECISION
...
#[test_only]
public fun destroy_pool<StakeCoin, RewardCoin>(pool: RewardPool<StakeCoin, RewardCoin>) {
    let RewardPool {
        id,
        total_stake: _,
