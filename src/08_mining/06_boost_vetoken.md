...
public fun lock_for_boost(
    ...
    let normalized_duration = if (duration_ms > max_duration_ms) { max_duration_ms } else {
        duration_ms
    };
...
fun get_user_boost_weight(pool: &BoostPool<_, _>, user: address, clock: &Clock): u64 {
...
fun update_pool(pool: &mut BoostPool<_, _>, clock: &Clock) {
    let now = clock.timestamp_ms();
    ...
    if (pool.total_boost_weight > 0) {
        pool.acc_boost_per_weight =
            pool.acc_boost_per_weight + boost_reward * PRECISION / pool.total_boost_weight;
    };
...
public fun pending<StakeCoin, RewardCoin>(
    ...
    let base_pending =
        pos.stake_amount * pool.acc_base_per_share / PRECISION - pos.base_reward_debt;
    let bw = get_user_boost_weight(pool, user, clock);
...
public fun vote(ve: &mut VoteEscrow, gauge_id: address, weight: u64, ctx: &mut TxContext) {
